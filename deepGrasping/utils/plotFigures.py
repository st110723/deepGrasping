...
class Debugging:
    ...
    def saveImageAfterSubBackgroundWithRect(self,  image_path=None, img_name=None, scene_id=None, ann_id=None):
        ...
        if self.dataset == "jacquard":
            img = self.openImage(os.path.join("processedData/imagesWithoutBackground", img_name))
            rec = self.dataPreprocessing.dataPreprocessing.getGraspingRectangles_2(img_name)
            
            for _, rectangle in rec.items():
                rectangleDrawing = RectangleDrawing(rectangle)
                rectangleDrawing.draw(np.float32(img))
            self.saveImage(img, img_name)
